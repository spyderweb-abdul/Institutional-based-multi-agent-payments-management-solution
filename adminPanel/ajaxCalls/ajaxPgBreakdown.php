...
require_once('../../include/constant_connection.php');
 ...
 $status = 'PAID';
	...
	$sql = $pgdb->query("SELECT feeItem, SUM(amount) AS amt, COUNT(admNo) AS admNo FROM fees_order WHERE payment_status='$status' AND session='$session' GROUP BY feeItem ");
...
echo '<td valign="top">';
 ...
 $eduerp = $edudb->query("SELECT title, SUM(price) AS price, COUNT(jambno) AS jambno FROM ".TBL_FEES_ORDER." WHERE level_name < 100 AND status = 'payment_received' AND session = '$session' AND programme <> 'General Fee Structure' AND programme <> 'Matriculation' AND programme NOT LIKE '%Bachelor%' GROUP BY title ");	
