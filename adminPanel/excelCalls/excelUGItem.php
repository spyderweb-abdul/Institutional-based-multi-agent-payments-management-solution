...
// Original PHP code by Chirp Internet: www.chirp.com.au
  ...
  $filename = 'UG_'.strtoupper($title).'_REPORT-' . date('Ymd') . '.xls';
 ...
 $fetch = $edudb->query("SELECT jambno AS ADMISSION_NO, order_id AS INVOICE_NO, email AS RRR, fullnames AS DEPOSITOR, price AS AMOUNT_PAID, course AS DEPT, level_name AS LEVEL, status AS STATUS 
                       FROM ".TBL_FEES_ORDER." WHERE title = '$title' AND status = '$status' AND session = '$session' AND level_name >= 100 
