...
if(isset($_GET['facId']))
{
   ...
   $faculty = $_GET['faculty'];
		...
		// Original PHP code by Chirp Internet: www.chirp.com.au
		  ...
		  $filename = strtoupper($faculty).'_PAYMENT_REPORT-'.$current_session.'-'. time() . '.xls';
		 ...
		 $sel_fac_details = $paydb->query("SELECT b.userId AS USER_ID, e.user_name AS USERNAME, h.level AS LEVEL, c.amount AS AMOUNT, c.invoice AS INVOICE, c.transactionId AS TRANSACTION_ID, f.option_name AS GATEWAY_OPTION, d.setup_name AS SETUP_NAME, c.session AS SESSION, c.dateTime AS DATE_TIME 
		 	 FROM ".FACULTY." a 
		 	 INNER JOIN ".FEES_ORDER." b ON b.facId = a.facId
			 ...
			 while($row = $sel_fac_details->fetch_assoc())
			 {
			     if(!$report) 
				{
