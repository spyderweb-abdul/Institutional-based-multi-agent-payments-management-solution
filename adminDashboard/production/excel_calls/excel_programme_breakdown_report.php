...
if(isset($_GET['progId']))
{
   ...
   $programme = $_GET['programme'];
		...
		// Original PHP code by Chirp Internet: www.chirp.com.au
		  ...
		  $filename = strtoupper($programme).'_PAYMENT_REPORT-'.$current_session.'-'. time() . '.xls';
		 ...
		 $sel_prog_details = $paydb->query("SELECT b.userId AS USER_ID, e.user_name AS USERNAME, h.level AS LEVEL, c.amount AS AMOUNT, c.invoice AS INVOICE, c.transactionId AS TRANSACTION_ID, f.option_name AS GATEWAY_OPTION, d.setup_name AS SETUP_NAME, c.session AS SESSION, c.dateTime AS DATE_TIME 
		 	 FROM ".PROGRAMME." a 
			 ...
			 while($row = $sel_prog_details->fetch_assoc())
			 {
			     if(!$report) 
				{
