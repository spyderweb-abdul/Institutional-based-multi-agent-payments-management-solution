...
session_start();
    ...
    try
    {
      $check_usr = $paydb->query("SELECT userId FROM ".USERS." WHERE userId = '$userId' AND merchantId = '$merchantId' ") or throw_ex(mysqli_error($paydb));
	    ...
	    if(mysqli_num_rows($check_usr) > 0)
	    {
			    echo '<div class="alert alert-warning alert-dismissible fade in" role="alert style="color:#FFF"> 
			          ...
			          <i class="fa fa-warning"></i> User with ID: '.$userId.' already exist. Kindly change the User ID. 
			          </div>';
	    ...
	    else
	    {
		   ...
		   $insert_usr = $paydb->query("INSERT INTO ".USERS." (userId, user_name, user_email, user_phone, merchantId, roleId) VALUES ('$userId', '$user_name', '$user_email', '$user_phone', '$merchantId', '$roleId')" ) or throw_ex(mysqli_error($paydb));
