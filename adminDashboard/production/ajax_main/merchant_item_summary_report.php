                    ...
                    <div class="modal-body"> 
                      ...
                      <?php 
                          ...
                          if(isset($_GET['feeid']))
                          {
                              ...
                              $sel_item_details = $paydb->query("SELECT * FROM ".FEE_ITEMS." a INNER JOIN ".FEES_ORDER." b ON b.feeItem = a.feeItem
                                                                ...
                                                                WHERE a.feeID = '$feeID' AND b.merchantId = '$merchantId' AND b.session = '$current_session' AND b.status = 'PAID' GROUP BY b.userId ORDER BY h.level ") or die (mysqli_error($paydb));

                              $num = mysqli_num_rows($sel_item_details);

                              if($num > 0)
                              {

                                  $res = $sel_item_details->fetch_array();

                                   $feeItem = $res['feeItem'];
                                    

                                    echo '<div style="color: #333; text-align:center; font-size:11px; font-weight: 500">'.strtoupper($feeItem).' <i class="fa fa-angle-double-right"></i> '.$current_session.' <i class="fa fa-angle-double-right"></i> BREAKDOWN </div><br/>'; 

                                    echo '<div class="pull-right" style="padding: 10px;"> <a class="btn btn-sm btn-success" href="excel_calls/excel_item_breakdown_report.php?feeid='.$feeID.'"> <i class="fa fa-file-excel-o"></i> Export to Excel </a> </div>';
                        ...
                        ?>
                             ...
                             <?php 
                                  ...
                                  $sel_item_details->data_seek(0);

                                  $i = 1;

                                  while($r = $sel_item_details->fetch_array()){
