                    ...
                    <div class="modal-body"> 
                          ...
                          if(isset($_GET['setupid']))
                          {
                              ...
                              $sel_setup_details = $paydb->query("SELECT * FROM ".SETUP." a INNER JOIN ".PAYMENT_RECORDS." b ON b.setupId = a.setupId 
                                                                  INNER JOIN ".PAYMENT_CHOICE." c ON c.choiceId = a.choiceId INNER JOIN ".PAYMENT_TYPE." d ON d.typeId = a.typeId INNER JOIN ".USERS." e ON e.userId = b.userId INNER JOIN ".PAY_OPTIONS." f ON f.optionId = b.optionId INNER JOIN ".CHANNEL." g ON g.gatewayId = b.gatewayId WHERE a.setupId = '$setupId' AND a.merchantId = '$merchantId' AND b.session = '$current_session' AND b.status = 'PAID' ") or die (mysqli_error($paydb));
                              ...
                              $setup_name = $res['setup_name'];
                                ...
                                echo '<div class="pull-right" style="padding: 10px;"> <a class="btn btn-sm btn-success" href="excel_calls/excel_payment_setup_report.php?setup_id='.$setupId.'&setup_name='.urlencode($setup_name).'"> <i class="fa fa-file-excel-o"></i> Export to Excel </a> </div>';
