                    ...
                    <div class="modal-body"> 
                      ...
                      <?php 
                          ...
                          if(isset($_GET['progid']))
                          {
                              ...
                              $sel_level_details = $paydb->query("SELECT * FROM ".PROGRAMME." a INNER JOIN ".FEES_ORDER." b ON b.progId = a.progId
                                                                ...
                                                                WHERE a.progId = '$progId' AND b.level = '$level' AND b.merchantId = '$merchantId' AND b.session = '$current_session' AND b.status = 'PAID' GROUP BY b.userId ORDER BY h.level ") or die (mysqli_error($paydb));

                              $num = mysqli_num_rows($sel_level_details);

                              if($num > 0)
                              {

                                  $res = $sel_level_details->fetch_array();

                                   $programme = $res['programme'];


                                    echo '<div style="color: #333; text-align:center; font-size:11px; font-weight: 500"> LEVEL: '.$level.' <i class="fa fa-angle-double-right"></i> '.strtoupper($programme).' <i class="fa fa-angle-double-right"></i> '.$current_session.' <i class="fa fa-angle-double-right"></i> BREAKDOWN </div><br/>'; 

                                    echo '<div class="pull-right" style="padding: 10px;"> <a class="btn btn-sm btn-success" href="excel_calls/excel_level_breakdown_report.php?progId='.$progId.'&programme='.urlencode($programme).'&level='.$level.'"> <i class="fa fa-file-excel-o"></i> Export to Excel </a> </div>';
                        ...
                        ?>
                             ...
                             <?php 
                                  ...
                                  $sel_level_details->data_seek(0);

                                  $i = 1;

                                  while($r = $sel_level_details->fetch_array()){
