...
<html> 
 <body>
                ...
                <div ng-app="programmeReportCtrl">
                    ...
                    <div class="modal-body"> 
                      ...
                      <?php 
                          ...
                          if(isset($_GET['progid']))
                          {
                              ...
                              $sel_prog_details = $paydb->query("SELECT * FROM ".PROGRAMME." a INNER JOIN ".FEES_ORDER." b ON b.progId = a.progId
                                                                ...
                                                                WHERE a.progId = '$progId' AND b.merchantId = '$merchantId' AND b.session = '$current_session' AND b.status = 'PAID' GROUP BY b.userId ORDER BY h.level ") or die (mysqli_error($paydb));

                              $num = mysqli_num_rows($sel_prog_details);

                              if($num > 0)
                              {

                                  $res = $sel_prog_details->fetch_array();

                                   $programme = $res['programme'];
                                    

                                    echo '<div style="color: #333; text-align:center; font-size:11px; font-weight: 500">'.strtoupper($programme).' <i class="fa fa-angle-double-right"></i> '.$current_session.' <i class="fa fa-angle-double-right"></i> BREAKDOWN </div><br/>'; 

                                    echo '<div class="pull-right" style="padding: 10px;"> <a class="btn btn-sm btn-success" href="excel_calls/excel_programme_breakdown_report.php?progId='.$progId.'&programme='.urlencode($programme).'"> <i class="fa fa-file-excel-o"></i> Export to Excel </a> </div>';
                        ...
                        ?>
                             ...
                             <?php 
                                  ...
                                  $sel_prog_details->data_seek(0);

                                  $i = 1;

                                  while($r = $sel_prog_details->fetch_array()){
