      ...
      <?php 
                          ...
                          if(isset($_GET['setupid']))
                          {
                              ...
                              $sel_setup_details = $paydb->query("SELECT * FROM ".SETUP." a INNER JOIN ".PAYMENT_RECORDS." b ON b.setupId = a.setupId 
                                                                  INNER JOIN ".PAYMENT_CHOICE." c ON c.choiceId = a.choiceId INNER JOIN ".PAYMENT_TYPE." d ON d.typeId = a.typeId INNER JOIN ".USERS." e ON e.userId = b.userId INNER JOIN ".PAY_OPTIONS." f ON f.optionId = b.optionId INNER JOIN ".CHANNEL." g ON g.gatewayId = b.gatewayId WHERE a.setupId = '$setupId' AND a.merchantId = '$merchantId' AND b.session = '$current_session' AND b.status = 'PENDING' ") or die (mysqli_error($paydb));
