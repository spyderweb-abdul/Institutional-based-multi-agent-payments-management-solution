...
if(strlen($userId) == 6)
{
$sql = $pgdb->query("SELECT * FROM ".TBL_ACCOUNT." WHERE accountID = '$userId'");
$r = $sql->fetch_array();
if(mysqli_num_rows($sql) > 0)
{
	...
	$descr = 'UDUS PG APPLICATION FORM - '.$orderID;
...
<table width="273" border="0" cellspacing="5" cellpadding="5">
  ...
  <tr>
    <td>
        ...
        <input type="hidden" class="form-control styleTb" size="30" required="required" readonly="readonly" value="'.$orderID.'" name="orderID" id="orderID" data-toggle="tooltip" data-placement="bottom" title="INVOICE NO." />
...
{
	echo '<div class="alert alert-danger styleTb"> There is no match for this account ID </div>';
}

}
else
{
	echo '<div class="alert alert-danger styleTb"> Incomplete Account ID </div>';
}
?>