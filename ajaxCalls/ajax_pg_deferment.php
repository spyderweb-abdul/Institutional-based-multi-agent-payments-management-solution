...
$sql = $pgdb->query("SELECT * FROM ".TBL_PG_ADM." WHERE app_no = '$userId' OR admNo = '$userId' AND admNo <> ' '");
$r = $sql->fetch_array();
...
if($num > 0 || $num == 0)
{
	...
	$des = 'PG DEFERMENT FEE';
	$payType = 'PG DEFERMENT';
	
	$session = '2016/2017';
	
	$check = $paydb->query("SELECT * FROM ".TBL_PAYMENTS_RECORD." WHERE userId = '$userId' AND description = '$des' AND session = '$session' ");
	$chk = $check->fetch_array();	
	
	$orderID = '10'.substr(date('Y'), -2).rand(1000, 9999).rand(3000, 8999);
	...
	$descr = 'PG DEFERMENT FEE - '.$invoice;
...
<table width="273" border="0" cellspacing="5" cellpadding="5">
  <tr>
    <td>
        ...
        <input type="text" class="form-control styleTb" size="30" required="required" value="'.$name.'" name="name" id="name" data-toggle="tooltip" data-placement="bottom" title="CANDIDATE\'S NAME" placeholder="INSERT YOUR FULL NAME" />
        </div>
  ...
  <tr>
    <td>
        ...
        <input type="text" class="form-control styleTb" size="30" required="required" value="'.$phone.'" name="phone" id="phone" data-toggle="tooltip" data-placement="bottom" title="PAYER\'S PHONE NO" placeholder="INSERT YOUR PHONE NO" />
        </div>
  ...
  <tr>
    <td>
        ...
        <input type="text" class="form-control styleTb" size="30" value="'.$email.'" name="email" id="email" data-toggle="tooltip" data-placement="bottom" title="PAYER\'S EMAIL ADDRESS" placeholder="INSERT YOUR EMAIL ADDRESS" />
        </div>
     ...
     <div class="input-group">
        ...
        <span class="input-group-addon styleTb"><b class="glyphicon glyphicon-shopping-cart"></b></span>
		 <select class="form-control styleTb" title="Credit Card Type" name="paymenttype" id="paymenttype" autocomplete="off">
						...
						<option selected="selected" value="BANK_BRANCH">BANK BRANCH</option>
...
{
	echo '<div class="alert alert-danger styleTb"> No Match for this APP. or ADM. No. Kindly check the number you have entered. </div>';
}
