...
$pay = $paydb->query("SELECT amount FROM ".TBL_CIS_FEE." WHERE prog = '$prog' AND level = '$level'");
$p =  $pay->fetch_array();
...
$r =   $sql->fetch_array();
	...
	$des = 'CIS REGISTRATION FEE';
	$payType = 'CIS REGISTRATION';
	...
	$orderID = '14'.substr(date('Y'), -2).rand(1000, 9999).rand(3000, 8999);
	...
	$descr = 'CIS REGISTRATION FEE - '.$invoice;
...
<table width="273" border="0" cellspacing="5" cellpadding="5">
  ...
  <tr>
	...
	<div class="input-group">
        ...
        <input type="text" class="form-control styleTb" size="30" required="required" value="'.$amt.'" name="amount" id="amount" data-toggle="tooltip" data-placement="bottom" title="PAYABLE AMOUNT" placeholder="ENTER AMOUNT (e.g: 5000)" />
        </div>
