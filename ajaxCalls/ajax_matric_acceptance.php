...
if(strlen($userId) == 10)
{
$sql = $matdb->query("SELECT * FROM ".TBL_BIODATA." WHERE appID = '$userId'");
$r =   $sql->fetch_array();
...
if(mysqli_num_rows($sql) > 0)
{
	...
	$invoice = '33'.rand(3000, 7777).rand(2000, 8865);
	$amt = '1000';
	$payment_status = 'PENDING';
	
	$descr = 'MATRICULATION ADMISSION ACCEPTANCE FEE - '.$invoice;
	$des = 'MATRICULATION ADMISSION ACCEPTANCE FEE';
	$payType = 'MATRIC ACCEPTANCE';
...
{
	echo '<div class="alert alert-danger styleTb"> There is no match for this Application No. </div>';
}

}
else
{
	echo '<div class="alert alert-danger styleTb"> Incomplete Application Number. </div>';
}
?>